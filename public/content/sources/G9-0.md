---
title: "DeepSeek-R1: Incentivizing Reasoning Capability in LLMs via RL (original paper)"
author: "arXiv, 2025"
source: "arXiv, 2025"
...
> Originally published by arXiv, 2025, arXiv, 2025 — https://arxiv.org/abs/2501.12948
...
https://huggingface.co/deepseek-ai. 2. DeepSeek-R1-Zero
...
For each question푞, GRPO samples a group of outputs{표
1
,표
2
,···,표
...
휃
표푙푑
and then optimizes the policy model 휋
휃
...
where휋
푟푒푓
is a reference policy,휀and훽are hyper-parameters, and퐴
푖
is the advantage, computed
using a group of rewards{푟
1
,푟
2
, . . . ,푟
...
Question: If 푎 > 1, then the sum of the real solutions of
√︁
푎−
√
푎+ 푥= 푥 is equal to
...
푎−
√
푎+ 푥= 푥, let’s start by squaring both···

√︁
푎−
√
...
Dev2, and Dev3 represent intermediate checkpoints within this pipeline. 3. DeepSeek-R1
...
helpfulness dataset. The general reward,푅푒푤푎푟푑
퐺푒푛푒푟푎푙
, assigned to each query corresponds to
...
ratio휀to 10, and the sampling temperature to 1 for rollout. For each question, we sample 16
...
Chinese
CLUEWSC (EM)
93.192.892.691.692.8
C-Eval (EM)
92.885.791.986.491.8
...
enhance the scope and accuracy of machine-driven solutions. 7. Author List
...
hyperparameter tuning—particularly of the휆coefficient in GAE—and is highly sensitive to this
parameter. When휆is set to 0.95 (the default value in most open-source PPO implementations),
PPO performs considerably worse than GRPO. However, with careful tuning (setting휆to 1.0),
...
Data Type# PromptsQuestion TypeOutput Type
Math26KQuantitative ReasoningNumber/Expression/Equation
Code17KAlgorithm and Bug FixingCode Solution
STEM
22KMulti-ChoiceOption
...
exam questions and competition problems. The average number of prompt tokens is 122. The dataset covers various mathematical domains such as algebra, calculus, proba-
...
matical expression (e.g., “푥
2
+3푥−2”), or an equation (e.g., “푦=2푥+1”). Mathematical
...
- Be clean and concise. Avoid colloquial language. Do not use phrases like "thought
  ...
  with correct final answers and a readable format. For mathematical outputs, we usesympy
...
testcase = [generate_random_string(90000, 100000) for * in range(t)]
print(t)
print("\n".join(testcase))
‘‘‘
...
char = random.choice(string.ascii*lowercase)
seq = char \* length
k = random.randint(1, length)
return f"{{seq}}\n{{k}}"
t = 1 # the number of test cases
testcase = [generate_single_repeated_char(90000, 100000) for * in range(t)]
print(t)
print("\n".join(testcase))
‘‘‘
...
import random
import string
def generate*alternating_chars(l_min, l_max):
length = random.randint(l_min, l_max)
...
return f"{{seq}}\n{{k}}"
t = 1 # the number of test cases
testcase = [generate_alternating_chars(90000, 100000) for * in range(t)]
print(t)
print("\n".join(testcase))
‘‘‘
...
2. ’correctness’: correct/incorrect
   ...
   Reasoning DataWe curate a large set of reasoning prompts and generate reasoning trajectories
...
For positive integer $n$, define $S_n$ to be the minimum value of the sum \[ \sum\_{k
...
Simplify: \( 1 - \lambda^2 = \lambda^2 \cdot \frac{(2k - 1)^2}{a*k^2} \).
...
\( \sum*{k=1}^n a*k = \sum*{k=1}^n \frac{2k - 1}{c} = \frac{1}{c} \sum*{k=1}^n (2k - 1)
\).
Earlier, I found that \( \sum*{k=1}^n (2k - 1) = n^2 \). Therefore,
\( \frac{n^2}{c} = 17 \) => \( c = \frac{n^2}{17} \).
Thus, \( a*k = \frac{2k - 1}{c} = \frac{(2k - 1) \cdot 17}{n^2} \).
Now, going back to the original expression for \( S_n \):
\( S_n = \sum*{k=1}^n \sqrt{(2k - 1)^2 + a*k^2} \).
...
\( S_n = \sqrt{1 + \left( \frac{17}{n^2} \right)^2 } \cdot \sum*{k=1}^n (2k - 1) \).
We already know that \( \sum\_{k=1}^n (2k - 1) = n^2 \), so this simplifies to:
...
To solve the problem, we need to find a positive integer \( n \) such that \( S*n \),
the minimum value of the sum \( \sum*{k=1}^n \sqrt{(2k-1)^2 + a*k^2} \), is an
integer. Here, \( a_1, a_2, \ldots, a_n \) are positive real numbers whose sum is 17.
...
total vertical distance is the sum of the vertical components, which is \( \sum*{k
...
Substituting this back into the expression for \( S*n \), we get:
\[ S_n = \sum*{k=1}^n \sqrt{(2k - 1)^2 + \left( \frac{17(2k - 1)}{n^2} \right)^2 } = \
sqrt{n^4 + 289} \]
...
So in the **init** method, I should initialize an empty dictionary. Like self.entries =
{}.
...
So, to implement:
Start by adding the **init**, newentry, and look methods as described.
</think>
...
2. **Initialization**: In the ‘**init**‘ method, initialize an empty dictionary to
...
1. **Initialization**: The ‘**init**‘ method sets up an empty dictionary ‘self.entries‘
...
starting at 5×10
−5
and gradually decreasing to 5×10
−6
. The maximum context length is set to
...
one-tenth of its initial value. The maximum context length is 32,768 tokens, and the batch size is 64.
...
rates and significant variability across different checkpoints. Therefore, we default to pass@푘
evaluation (Chen et al., 2021) and report pass@1 using a non-zero temperature. Specifically, we
use a sampling temperature of 0.6 and a top-푝value of 0.95 to generate푘responses (typically
between 4 and 64, depending on the test set size) for each question. Sepcifically, we use푘=64
for AIME and GPQA,푘=16 for MATH and CodeForces, and푘=8 for LCB. Pass@1 is then
40
...
where푝
푖
denotes the correctness of the푖-th response. This method provides more reliable
...
denote the performance is statistically significant (t−test with 푝 < 0.01).
Benchmark (Metric)
Claude-3.5- GPT-4o DeepSeekOpenAI OpenAIDeepSeek
Sonnet-10220513V3o1-mini o1-1217R1
...
50.0
81.2
DeepSeek-R1DeepSeek-R1-ZeroHuman Expert
...
unsafe dialogues.
Model-based Risk ReviewSubsequently, these potentially unsafe dialogues are concatenated
...
higher score indicates better safety performance. Benchmarks marked with _ are the results
...
Safety Score(%)SST BBQ ART XSTest DNA_ HarmBench\*Average Score
Claude-3.7-Sonnet100.0 92.1 99.796.495.983.394.6
o1 (2024-12-17)99.0 97.3 98.397.086.284.093.6
GPT-4o (2024-05-13)
98.5 95.1 99.197.390.672.792.2
Qwen2.5 Instruct (72B)100.0 95.4 99.697.995.983.095.3
DeepSeek-V395.3 96.7 97.197.195.696.0 (67.0)96.3 (91.5)
DeepSeek-R1 (hide cot)98.0 96.6 97.294.493.796.3 (58.0)96.0 (89.7)
DeepSeek R197.5 96.6 96.295.394.889.3 (35.0)95.0 (85.9)
...
Behavior
Gender and Sexual DiscriminationPersonal Health Discrimination
Appearance and Body Shape Discrimination
Personal Social Status DiscriminationNational and Regional Discrimination
Ethnic and Racial DiscriminationReligious Discrimination
...
Economic CrimesCyber Crimes
Animal-Related Crimes
Physical HarmPsychological Harm
Privacy ViolationsEconomic Rights Violations
Other Legal Rights Violations
Family EthicsMarriage Ethics
Academic EthicProfessional Ethics
...
Illegal and Criminal BehaviorIllegal activities encompass the following safety topics: violent
...
Moral and Ethical IssuesWe have identified and categorized the ethical issues in the following
...
Discrimi.IllegalHarmfulEthicalOverall
Ratio(%)Unsafe Rej.Unsafe Rej.Unsafe Rej.Unsafe Rej.Unsafe Rej.
Claude-3.7-Sonnet8.42.514.14.59.55.57.50.610.73.6
o1 (2024-12-17)
7.237.812.354.85.073.58.834.49.050.4
GPT-4o (2024-05-13)19.16.222.528.428.019.518.84.422.017.1
Qwen2.5 Instruct (72B)
12.82.514.59.515.55.011.90.013.85.4
DeepSeek-V320.32.517.313.917.59.513.11.917.68.1

- risk control system8.116.93.235.57.022.53.118.15.325.4
  DeepSeek-R119.73.828.98.632.56.016.90.625.25.6
- risk control system
  9.117.26.639.113.029.06.913.18.527.3
...
ClaudeGPT4oV3V3-CheckR1R1-Check
model
60
70
80
...
Ratio(%)Origin Jailbreak GAPOrigin Jailbreak GAP
...
Social SciencesSTEMOtherHumanities
80.0
82.5
85.0
87.5
...
statistically significant (t−test with 푝 < 0.01).
Benchmark (Metric)V3-Base V3 R1-Zero R1
English
MMLU (EM)87.188.588.890.8
MMLU-Redux (EM)86.289.185.692.9
MMLU-Pro (EM)64.475.968.984.0
DROP (3-shot F1)
89.091.689.192.2
...
SWE Verified (Resolved)-42.043.249.2
...
5_AIME_II_Problems
), DeepSeek-R1 achieves a 75% solve rate (Pass@1), approaching o1’s
...
- a performance that, when combined with its AIME results, yields a score exceeding the
  ...
  Olympiadhttps://artofproblemsolving.com/wiki/index.php/AMC_historical_
  ...
  //artofproblemsolving.com/community/c3752401_2024_contests
  ), including
...
Difficulty Level
DeepSeek-R1 DeepSeek-R1 DeepSeek-R1 DeepSeek-R1DeepSeek
ZeroDev1Dev2Dev3R1
...
foundation models such as Qwen (Qwen, 2024b) and LLaMA (AI@Meta, 2024; Touvron et al., 2023) using a curated dataset comprising 800,000 samples generated with DeepSeek-R1. Details
...
significant (t−test with 푝 < 0.01).
Model
AIME 2024MATH
GPQALiveCode
CodeForces
DiamondBench
pass@1 cons@64 pass@1 pass@1 pass@1rating
...
PROMPT
Question: There is a stack of N cards, and the ith card from the top has an integer 퐴
푖
written on it.
...
Let퐵
푖
be the integer written on the ith card from the top of the stack after the operation. Print
퐵
1
, 퐵
2
...
age **_, cause unnecessary _** or **_, and must not cause _** offence.
...
“‘sql SELECT _ FROM ( SELECT _ FROM table1 LIMIT 10 ) AS t1 JOIN table2 ON t1.code = table2.code
...
1. Assistant A is significantly better: [[A>>B]]
2. Assistant A is slightly better: [[퐴 >B]]
3. Tie, relatively the same: [[A=B]]
4. Assistant B is slightly better: [[B>A]]
5. Assistant B is significantly better: [[B>>A]]
...
勇
——尽管她们对他都很熟悉；当然也绝不会愚蠢地提出蒋盈波今后是一个人过到底还是再找个老
...
信号显示，金雕仅在放飞地
3公里的范围内飞行。以下哪项如果为真，最有助于解释上述金雕的行
为?_**
A.金雕放飞地周边重峦叠嶂，险峻异常。
...
Two quantum states with energies E1 and E2 have a lifetime of 10
−9
sec and 10
−8
sec, respectively.
...
PROMPT
Let푏⩾2 be an integer. Call a positive integer푛 푏-eautifulif it has exactly two digits when expressed
in base푏, and these two digits sum to
√
푛. For example, 81 is 13-eautiful because 81= 63
13
and
6+ 3=
√ 81. Find the least integer 푏⩾ 2 for which there are more than ten 푏-eautiful integers.
Please reason step by step, and put your final answer within \boxed{}.
Evaluation
Parse the final answer within \boxed{} and use a rule-based grader to determine if it equals the
ground truth. Round numerical values as needed, and use ‘SymPy’
1
to parse expressions.
78

References
AI@Meta. Llama 3.1 model card, 2024. URLhttps://github.com/meta-llama/llama-m
...
test-time training for abstract reasoning.arXivpreprintarXiv:2411.07279, 2024.
...
human feedback.arXivpreprintarXiv:2204.05862, 2022.
B. Brown, J. Juravsky, R. Ehrlich, R. Clark, Q. V. Le, C. Ré, and A. Mirhoseini. Large language mon-
keys: Scaling inference compute with repeated sampling.arXivpreprintarXiv:2407.21787, 2024.
...
onNeuralInformationProcessingSystems2020,NeurIPS2020,December6-12,2020,virtual, 2020. URLhttps://proceedings.neurips.cc/paper/2020/hash/1457c0d6bfcb4
...
et al. An empirical study on eliciting and improving r1-like reasoning models.arXivpreprint
...
R. Fergus, S. V. N. Vishwanathan, and R. Garnett, editors,AdvancesinNeuralInformation
ProcessingSystems30:AnnualConferenceonNeuralInformationProcessingSystems2017,
December4-9,2017,LongBeach,CA,USA, pages 4299–4307, 2017. URLhttps://procee
...
CMS. Chinese national high school mathematics olympiad, 2024. URLhttps://www.cms.or
...
model.CoRR, abs/2405.04434, 2024a. URLhttps://doi.org/10.48550/arXiv.2405. 04434.
DeepSeek-AI. Deepseek-v3 technical report.
arXivpreprintarXiv:2412.19437, 2024b.
H. Face. Open r1: A fully open reproduction of deepseek-r1, January 2025. URLhttps:
//github.com/huggingface/open-r1.
X. Feng, Z. Wan, M. Wen, S. M. McAleer, Y. Wen, W. Zhang, and J. Wang. Alphazero-like
tree-search can guide large language model decoding and training, 2024. URLhttps:
...
P. Gauthier. Aider LLM leaderboard, 2025. URLhttps://aider.chat/docs/leaderboar
ds/.
...
depth approach.arXivpreprintarXiv:2502.05171, 2025.
...
forComputationalLinguistics:HumanLanguageTechnologies,NAACL2025-Volume1:
LongPapers,Albuquerque,NewMexico,USA,April29-May4,2025, pages 5069–5096.
Association for Computational Linguistics, 2025. URLhttps://aclanthology.org/2025.
naacl-long.262/.
80
...
language models can self-correct with tool-interactive critiquing. InTheTwelfthInternational
...
integrated reasoning agent for mathematical problem solving. InTheTwelfthInternational
...
is planning with world model. InThe2023ConferenceonEmpiricalMethodsinNatural
LanguageProcessing, 2023. URL https://openreview.net/forum?id=VTWWvYtF1R.
...
massive multitask language understanding. In9thInternationalConferenceonLearning
...
inNeuralInformationProcessingSystems36:AnnualConferenceonNeuralInformation
ProcessingSystems2023,NeurIPS2023,NewOrleans,LA,USA,December10-16,2023, 2023. URLhttp://papers.nips.cc/paper_files/paper/2023/hash/c6ec1844bec
...
reasoners. In A. H. Oh, A. Agarwal, D. Belgrave, and K. Cho, editors,AdvancesinNeural
...
V. Srikumar, editors,FindingsoftheAssociationforComputationalLinguistics,ACL2024,
Bangkok,Thailandandvirtualmeeting,August11-16,2024, pages 11260–11285. Association
...
code input-output prediction.arXivpreprintarXiv:2502.07316, 2025.
H. Lightman, V. Kosaraju, Y. Burda, H. Edwards, B. Baker, T. Lee, J. Leike, J. Schulman,
I. Sutskever, and K. Cobbe. Let’s verify step by step. InTheTwelfthInternationalConference
onLearningRepresentations,ICLR2024,Vienna,Austria,May7-11,2024. OpenReview.net, 2024. URL https://openreview.net/forum?id=v8L0pN6EOi.
...
MAA. American invitational mathematics examination - aime. InAmericanInvitational
MathematicsExamination-AIME2024, February 2024. URLhttps://maa.org/math
...
P. Clark. Self-refine: Iterative refinement with self-feedback. InThirty-seventhConferenceon
...
for Automated Red Teaming and Robust Refusal. InForty-firstInternationalConferenceon
...
C. Raffel. Scaling data-constrained language models. InThirty-seventhConferenceonNeural
...
OpenAI. GPT4 technical report.arXivpreprintarXiv:2303.08774, 2023.
OpenAI. Introducing SimpleQA, 2024a. URLhttps://openai.com/index/introducing
-simpleqa/.
OpenAI. Introducing SWE-bench verified we’re releasing a human-validated subset of swe-
bench that more, 2024b. URLhttps://openai.com/index/introducing-swe-bench
-verified/.
...
S. R. Bowman. BBQ: A hand-built bias benchmark for question answering. InFindingsof
theAssociationforComputationalLinguistics:ACL2022,Dublin,Ireland,May22-27,2022,
pages 2086–2105. Association for Computational Linguistics, 2022.
Qwen. Qwq: Reflect deeply on the boundaries of the unknown, 2024a. URLhttps://qwenlm
.github.io/blog/qwq-32b-preview/.
Qwen. Qwen2.5: A party of foundation models, 2024b. URLhttps://qwenlm.github.io/b
log/qwen2.5.
A. Radford, J. Wu, R. Child, D. Luan, D. Amodei, I. Sutskever, et al. Language models are
unsupervised multitask learners.OpenAIblog, 1(8):9, 2019.
...
A. Globerson, K. Saenko, M. Hardt, and S. Levine, editors,AdvancesinNeuralInformation
...
Identifying Exaggerated Safety Behaviours in Large Language Models. InProceedingsof
the2024ConferenceoftheNorthAmericanChapteroftheAssociationforComputational
Linguistics:HumanLanguageTechnologies(Volume1:LongPapers),NAACL2024,Mexico
83

City,Mexico,June16-21,2024, pages 5377–5400. Association for Computational Linguistics, 2024.
...
tools. InThirty-seventhConferenceonNeuralInformationProcessingSystems, 2023. URL
https://openreview.net/forum?id=Yacmpz84TH.
J. Schulman. Approximating kl divergence, 2020. URLhttp://joschu.net/blog/kl-app
rox.html.
J. Schulman, P. Moritz, S. Levine, M. Jordan, and P. Abbeel. High-dimensional continuous
control using generalized advantage estimation.arXivpreprintarXiv:1506.02438, 2015.
...
onMachineLearningResearch, 2024. ISSN 2835-8856. URLhttps://openreview.net/f
...
effective than scaling model parameters, 2024. URLhttps://arxiv.org/abs/2408.033 14.
C. V. Snell, J. Lee, K. Xu, and A. Kumar. Scaling LLM test-time compute optimally can be more
effective than scaling parameters for reasoning. InTheThirteenthInternationalConference
...
for generalization under distribution shifts. InInternationalconferenceonmachinelearning,
...
solve them. In A. Rogers, J. Boyd-Graber, and N. Okazaki, editors,FindingsoftheAssociation
forComputationalLinguistics:ACL2023, pages 13003–13051, Toronto, Canada, July 2023.
...
free step-by-step verifier for llms in mathematical reasoning.arXivpreprintarXiv:2312.08935,
2023a.
X. Wang, J. Wei, D. Schuurmans, Q. V. Le, E. H. Chi, S. Narang, A. Chowdhery, and D. Zhou.
Self-consistency improves chain of thought reasoning in language models. InTheEleventh
InternationalConferenceonLearningRepresentations,ICLR2023,Kigali,Rwanda,May1-5, 2023. OpenReview.net, 2023b. URL https://openreview.net/forum?id=1PL1NIMMrw.
X. Wang, J. Wei, D. Schuurmans, Q. V. Le, E. H. Chi, S. Narang, A. Chowdhery, and D. Zhou.
Self-consistency improves chain of thought reasoning in language models. InTheEleventh
InternationalConferenceonLearningRepresentations,ICLR2023,Kigali,Rwanda,May1-5, 2023. OpenReview.net, 2023c. URL https://openreview.net/forum?id=1PL1NIMMrw.
...
D. Belgrave, A. Fan, U. Paquet, J. M. Tomczak, and C. Zhang, editors,AdvancesinNeural
InformationProcessingSystems38:AnnualConferenceonNeuralInformationProcessing
Systems2024,NeurIPS2024,Vancouver,BC,Canada,December10-15,2024, 2024. URL
...
abilities of large language models.Trans.Mach.Learn.Res., 2022, 2022a. URLhttps:
...
hamed, A. Agarwal, D. Belgrave, K. Cho, and A. Oh, editors,AdvancesinNeuralInformation
ProcessingSystems35:AnnualConferenceonNeuralInformationProcessingSystems2022,
NeurIPS2022,NewOrleans,LA,USA,November28-December9,2022, 2022b. URL
...
quences by learning to self-correct. InTheEleventhInternationalConferenceonLearning
...
engineering agents.arXivpreprint, 2024.
...
Deliberate problem solving with large language models. InThirty-seventhConferenceon
...
reasoning and acting in language models. InTheEleventhInternationalConferenceon
...
ship on learning mathematical reasoning with large language models.arXivpreprint
arXiv:2308.01825, 2023.
E. Zelikman, Y. Wu, J. Mu, and N. Goodman. STar: Bootstrapping reasoning with reasoning.
In A. H. Oh, A. Agarwal, D. Belgrave, and K. Cho, editors,AdvancesinNeuralInformation
ProcessingSystems, 2022. URL https://openreview.net/forum?id=_3ELRdg2sgI.
E. Zelikman, G. R. Harik, Y. Shao, V. Jayasiri, N. Haber, and N. Goodman. Quiet-STar: Language
models can teach themselves to think before speaking. InFirstConferenceonLanguage
...
evaluation for large language models.arXivpreprintarXiv:2311.07911, 2023b.
86
