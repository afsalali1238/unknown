...
const updates = {
  ...
  C5: {
    url: "https://blog.syzgroup.com/slow-food-for-thought/michael-burry-beyond-the-big-short",
    label: "Michael Burry beyond The Big Short",
    source: "Syz Group",
  },
  C6: {
  ...
  D6: {
    ...
    label: "Marks' own follow-up: Further Thoughts on Sea Change",
  ...
  I9: {
    ...
    label: "Notes & reception of The Sovereign Individual",
    source: "Wikipedia",
  },
  I10: {
...
content = content.replace(
  /(id: "D4",\s*clusterId: "D",\s*title: "Waiting for the Last Dance",\s*author: )"Howard Marks"/,
  '$1"Jeremy Grantham"',
);

for (const [id, data] of Object.entries(updates)) {
  const nodeRegex = new RegExp(
    `(id:\\s*"${id}"[\\s\\S]*?furtherReading:\\s*\\[\\s*\\{[\\s\\S]*?)label:\\s*"[^"]*",\\s*source:\\s*"[^"]*",\\s*url:\\s*"https://scholar\\.google\\.com/scholar\\?q=[^"]*"`,
    "g",
  );
  ...
  if (!nodeRegex.test(content)) {
    ...
    const fallbackRegex = new RegExp(
      `(id:\\s*"${id}"[\\s\\S]*?furtherReading:\\s*\\[\\s*\\{[\\s\\S]*?)label:\\s*"[^"]*",\\s*source:\\s*"[^"]*",\\s*url:\\s*"[^"]*"`,
      "g",
    );
    if (fallbackRegex.test(content)) {
      content = content.replace(
        fallbackRegex,
        `$1label: ${JSON.stringify(data.label)},\n        source: ${JSON.stringify(data.source)},\n        url: ${JSON.stringify(data.url)}`,
      );
