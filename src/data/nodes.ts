...
export const NODES: Node[] = [
  // A — Startup Fundamentals
  {
    ...
    layer0:
      "Early on, you can't rely on a growth engine because you don't have one yet. You have to recruit users personally, one at a time, and give them a delightful experience — even if that means doing tasks manually behind the scenes.",
    layer1:
      "The reason unscalable work matters is that it produces two things scalable work can't: real signal from a small number of users, and momentum. Manual effort lets you learn quickly what people want, and it creates the initial base that later engines amplify. Airbnb photographing apartments themselves and Stripe onboarding users in person are canonical examples.",
    layer2:
      "Pick the smallest ambitious group of users you could serve extraordinarily well this week. Do their work manually — greet them, fix their bugs, hand-deliver value. Notice which requests you keep serving; those are your product roadmap.",
    quiz: {
      question:
        "A founder refuses to onboard users manually because it 'won't scale.' What's the strongest counter?",
      options: [
  ...
  },
  {
    ...
    category: "Startups",
    thesis:
      "The only thing that matters is being in a good market with a product that satisfies it.",
    layer0:
      "Product/market fit is when you're in a market that pulls the product out of your hands — servers melt, salespeople can't answer the phones, users show up faster than you can support them. Before that moment, nothing else matters much. After it, almost nothing else matters either.",
    layer1:
      "Andreessen's insight is that team, product, and market are the three inputs, but market dominates. A great team with a mediocre product in a huge market will figure it out; a great team with a great product in a nonexistent market will die. PMF is felt, not measured — you know it because demand outruns supply.",
    layer2:
      "For your current work: is the market pulling, or are you pushing? If you're pushing (chasing users, low retention), stop building and change the target — a different customer, a different problem, or a different product for the same problem.",
    quiz: {
      question:
        "You have a beautifully engineered product but retention is 5% at day 30. What does Andreessen say to do?",
      options: [
  ...
  },
  {
    ...
    layer0:
      "At any given moment, a startup is either default alive — will reach profitability on current growth and burn — or default dead. Founders often can't answer which they are, and that's the danger. The question forces honesty about runway, growth rate, and expenses.",
    layer1:
      "The math: project your current expense trajectory and current revenue growth. If revenue overtakes expenses before cash runs out, you're default alive. The trap is assuming your next raise will save you; markets close, and 'default dead but hoping' is not a strategy — it's a bet on outside conditions.",
    layer2:
      "Run the calculation on your own project or company today. Write down cash on hand, monthly burn, monthly revenue growth. If default dead, decide now: cut costs or grow faster. Don't drift.",
    quiz: {
    ...
    furtherReading: [
      {
        label: "Default Alive or Default Dead",
        source: "paulgraham.com",
  ...
  },
  {
    ...
    layer0:
      "When people say 'I have an idea,' they usually mean a one-liner. But real ideas are mazes: dozens of decisions about who, how, and when, each of which has already been attempted by someone. A founder who has walked the maze knows which paths dead-end and which are unexplored.",
    layer1:
      "Every startup idea has a history — previous companies, adjacent markets, technology cycles, regulatory shifts. Studying the maze reveals why now, why this angle, and what specifically has changed to make the previously-impossible now possible. This is why domain knowledge beats generic pattern matching.",
    layer2:
      "For an idea you care about, list five prior attempts. For each, name why it failed and what has changed since. If you can't fill this in, you haven't walked the maze yet.",
    quiz: {
    ...
    furtherReading: [
      {
        label: "Market Research, Wireframing, and Design",
        source: "cs183c",
        url: "https://cs183c.stanford.edu/",
  ...
  // F — Crypto
  {
    ...
    layer0:
      "In nine pages, Satoshi proposed a peer-to-peer electronic cash system: transactions broadcast to a network, ordered into blocks, and secured by computational work so that rewriting history costs more than it could ever earn. No bank, no central server, no permission.",
    layer1:
      "The problem Bitcoin solves is double-spending without a trusted mediator. Its answer is a chain of blocks where each block references the last and is stamped with a hard-to-produce hash. Honest miners always work on the longest chain; an attacker would need more compute than the entire honest network combined to rewrite recent history — economically absurd at scale.",
    layer2:
      "Read the whitepaper (nine pages, mostly plain English). Try to explain, in your own words, why the longest chain rule makes attacks economically self-defeating.",
    quiz: {
  ...
  },
  {
    ...
    clusterId: "F",
    title: "Proof of Work",
    author: "Adam Back / Satoshi Nakamoto",
    year: 1997,
    medium: "Model",
    category: "Crypto",
    thesis: "Turn energy into ordered history that no one can rewrite cheaply.",
    layer0:
      "Proof of work asks computers to find a number that, combined with a block of data, produces a hash starting with many zeros. Finding it takes energy; verifying it is instant. That asymmetry lets a decentralized network agree on which history is real: the one with the most work behind it.",
    layer1:
      "PoW's genius is aligning security with cost. To attack the chain, you must out-spend the honest network on hardware and electricity forever. The critique — energy waste — is also the feature: the energy is what makes the security real and non-fake-able. Newer alternatives (proof of stake) trade energy for capital lockup; each has different attack surfaces.",
    layer2:
      "Explain to a non-technical friend why proof of work is not 'wasted' electricity from a security standpoint. If you can't, you don't fully understand it yet — try again with a physical analogy.",
    quiz: {
  ...
  },
  {
    ...
    layer0:
      "Lessig argued that in cyberspace, software architecture — not just legislation — determines what is possible. In crypto, this became literal: smart contracts execute exactly what they say, no matter what the parties intended. The DAO hack of 2016 tested this when an attacker legally extracted funds by exploiting a bug.",
    layer1:
      "The problem is that intent and code diverge. Ethereum eventually hard-forked to reverse the DAO hack, splitting the community into ETH and ETC (Ethereum Classic). The fork proved code is not always law — sometimes the social layer overrides the technical one. The tension between those two layers is a permanent feature of crypto governance.",
    layer2:
      "For a smart contract or automated policy in your work, ask: what would happen if it executed exactly as written but obviously against everyone's intent? Is there a social layer that can override, or are you truly bound?",
    quiz: {
    ...
    furtherReading: [
      {
        label: "Code and Other Laws of Cyberspace",
        source: "book",
  ...
  },
  {
    ...
    layer0:
      "Stablecoins try to hold a stable value (usually $1). They do it three ways: fiat-backed (USDC holds real dollars), crypto-backed (DAI over-collateralized in ETH), or algorithmic (LUNA/UST used market incentives). Each design trades off trust, capital efficiency, and failure mode.",
    layer1:
      "Fiat-backed depends on trusting the custodian to actually hold the dollars (Tether's saga). Crypto-backed depends on collateral not crashing faster than liquidations can happen. Algorithmic depends on a demand loop that can reverse into a death spiral — as Terra/UST showed in 2022, wiping out $40B in days. There's no free lunch: the peg is only as strong as its weakest failure mode.",
    layer2:
      "For any stablecoin you use, identify which of the three types it is and what would cause it to depeg. If you can't answer, don't hold more of it than you can lose.",
    quiz: {
  ...
  // G — Macro & Investing
  {
    ...
    layer0:
      "Graham's core rule: never rely on being exactly right. Estimate what something is worth, then require the price to be well below that estimate before buying. The gap — the margin of safety — protects you from bad estimates, bad luck, and bad markets.",
    layer1:
      "The margin exists because valuation is uncertain. Even careful analysis produces a range, not a point. If your best estimate is $100 and you demand a 30% margin, you buy at $70. That gives room for the estimate to be wrong. The principle generalizes far beyond investing: engineer bridges with safety factors, plan projects with slack, allow real relationships to breathe.",
    layer2:
      "For a decision on your plate, identify your best-case estimate and the price/cost you're paying. What's the margin of safety? If it's small, either wait or renegotiate.",
    quiz: {
  ...
  },
  {
    ...
    category: "Investing",
    thesis:
      "Treat the market as a moody partner who quotes prices, not as an oracle who tells you value.",
    layer0:
      "Graham imagined a business partner, Mr. Market, who shows up every day with a price at which he'll buy or sell his share. Some days he's euphoric and quotes a high price; other days depressed and quotes a low one. You are never obliged to trade — you can wait for the days his mood serves you.",
    layer1:
      "The parable reframes the market as an emotional counterparty rather than a source of truth. Prices contain information, but they also contain feeling. When feeling dominates, mispricing appears. The discipline is to have a view of value independent of the market's quote so you can act when the two diverge, and stay still when they don't.",
    layer2:
      "Pick an asset (or a decision). Estimate its value on your own — not by looking at the current price. Now compare. Which is higher? What does that tell you about acting today?",
    quiz: {
    ...
    furtherReading: [
      {
        label: "The Intelligent Investor, Ch. 8",
        source: "book",
        url: "https://www.harpercollins.com/products/the-intelligent-investor-benjamin-graham",
  ...
  },
  {
    ...
    layer0:
      "Traditional theory says prices reflect fundamentals. Soros observed that prices also influence fundamentals: a company whose stock is rising can raise cheap capital, hire, expand — improving the very fundamentals that justify the higher price. And in reverse. This is reflexivity.",
    layer1:
      "Reflexive loops explain bubbles and crashes better than efficient markets do. Rising prices attract narratives, capital, and confidence, which raise prices further, until the underlying reality can no longer support the belief — then the loop reverses, sometimes violently. Soros made his fortune identifying reflexive setups and betting on their turning points, especially where a policy peg was distorting fundamentals.",
    layer2:
      "For a market or trend you watch, ask: is the price driving the fundamentals, or the fundamentals driving the price? If the former, look for the point where the loop can no longer sustain itself.",
    quiz: {
  ...
  },
  {
    ...
    category: "Investing",
    thesis:
      "For a repeated bet with an edge, the growth-optimal fraction to wager is precise — and smaller than you think.",
    layer0:
      "If you bet too little on a favorable edge, you leave growth on the table. If you bet too much, variance can wipe you out even when the edge is real. Kelly gives the exact fraction that maximizes long-run compound growth: for a bet with probability p and payoff b, wager f = (bp - q) / b, where q = 1-p.",
    layer1:
      "Two things make Kelly humbling. First, most real-world edges are much smaller than intuition suggests, so optimal position sizes are much smaller than gut. Second, Kelly assumes you know p and b precisely; misestimation leads to catastrophic overbetting. Most practitioners use 'half Kelly' — it gives up some growth for far more robustness to model error.",
    layer2:
      "Take a bet you're considering. Estimate p and b. Apply Kelly. Notice how small the answer is versus your instinct. Consider whether you're over-betting your edge.",
    quiz: {
  ...
  },
  {
    ...
    layer0:
      "Dalio separates debt crises into two archetypes: deflationary deleveragings, typical of countries (like the US in 2008) whose debt is denominated in their own currency, and inflationary deleveragings, typical of emerging markets that borrowed in foreign currency. In either case, a crisis resolves through some combination of four levers: austerity (cutting spending), defaults and restructurings (writing off bad debt), redistributing wealth (taxation), and printing money (quantitative easing) — and the specific mix chosen determines whether the outcome is a deep, destructive depression or what Dalio calls a 'beautiful deleveraging.'",
    layer1:
      "The mechanism for a 'beautiful deleveraging' requires precisely calibrating the printing of new money to exactly offset the deflationary shock caused by credit and debt destruction — enough monetary stimulus to keep nominal economic growth above nominal interest rates, without printing so much that it triggers runaway inflation. Because austerity and defaults are inherently deflationary (they destroy money and demand), while printing money is inflationary (it creates new money), policymakers who understand this balance can, in Dalio's framework, engineer a much softer landing than crises have historically produced when handled by instinct rather than by this explicit template.",
    layer2:
      "Think about a period of financial stress you've lived through (personal, or a national economic event you followed closely). Which of Dalio's four levers — austerity, defaults, redistribution, or money printing — was actually used, and did the mix seem deliberate or improvised?",
    quiz: {
  ...
  },
  {
    ...
    layer0:
      "Pozsar's framework distinguishes between 'inside money' (sovereign fiat currencies and government bonds, which are simultaneously somebody else's liability and can therefore be frozen or confiscated by that issuer) and 'outside money' (gold, commodities, and raw materials, which are nobody's liability and can't be revoked by decree). By freezing Russia's dollar and euro reserves, the West demonstrated in real time that inside money is not the unconditionally safe asset it was assumed to be, once geopolitics enters the picture.",
    layer1:
      "The mechanism is a rational response by commodity-producing nations to a newly demonstrated vulnerability: if holding foreign reserves as 'inside money' (Treasuries, fiat deposits) can be frozen at the geopolitical discretion of the issuing country, then nations wary of similar treatment have an incentive to demand settlement in outside money instead — gold or commodity-backed currencies rather than sanctionable fiat. Pozsar's famous framing was that 'you can print money, but you can't print oil to heat or wheat to eat,' capturing the fatal flaw of relying on central bank liquidity to solve a shock rooted in physical commodity supply chains rather than pure financial liquidity.",
    layer2:
      "Consider an asset or arrangement you rely on that seems safe today only because the entity controlling it hasn't yet had a reason to restrict your access. What would you need to hold instead if you wanted an asset that couldn't be frozen or revoked by any single counterparty?",
    quiz: {
  ...
  },
  {
    ...
    layer0:
      "The deck's core message to founders was blunt: capital markets were closing, possibly for a long time, and any startup relying on a continuous stream of new VC funding to survive needed to immediately pivot toward generating its own cash. That meant cutting headcount, killing vanity projects and product lines that weren't core to the business, and ruthlessly focusing on unit economics — the actual profitability of each unit of the business, rather than just top-line user growth.",
    layer1:
      "The mechanism was forcing a reassessment of what 'success' meant for a startup under radically different funding conditions. In good times, growth metrics like user acquisition mattered more than profitability, because more funding was always assumed to be available down the road. Sequoia's argument was that this assumption had broken: startups needed to achieve 'default-alive' status, meaning they could survive on existing cash and revenue alone, without assuming any future fundraising would be possible. Companies like early Airbnb and Dropbox, which executed these cost-cutting directives, survived the downturn and went on to dominate the following decade — a real-world test of the thesis.",
    layer2:
      "If external funding for your current project, business, or even personal finances disappeared entirely starting tomorrow, what would you cut immediately to reach 'default-alive' status, and how long would your current resources actually last?",
    quiz: {
  ...
  },
  {
    ...
    layer0:
      "Sino-Forest claimed to own vast forestry assets in China and reported strong profit margins on timber sales. Block's team discovered the company routed its claimed purchases and sales of standing timber through opaque offshore intermediaries — entities that supposedly bought and sold the timber on Sino-Forest's behalf, conveniently also being the parties responsible for producing tax paperwork Sino-Forest's own auditors never quite managed to obtain. The physical reality didn't match the paper reality at all: local forestry bureau records showed timber volumes far below what Sino-Forest claimed to be harvesting and selling.",
    layer1:
      "The mechanism was cross-referencing corporate claims against independently verifiable physical and regulatory records. Sino-Forest reported timber sales in Yunnan province worth over $231 million, but the volume implied by that figure exceeded the region's legally permitted harvesting quota by roughly six times over — meaning the trees, at that scale, simply couldn't have existed. Muddy Waters calculated that transporting the claimed log volumes would have required tens of thousands of trucks operating simultaneously on narrow mountain roads, a logistical impossibility. The offshore 'Authorized Intermediaries' structure existed specifically to obscure this gap from auditors who never independently verified the underlying assets.",
    layer2:
      "For any company whose core asset is physical (real estate, inventory, natural resources, factories), ask: has anyone independently verified that asset exists at the scale claimed, using a source outside the company's own reporting — government records, satellite imagery, on-the-ground observation?",
    quiz: {
      question:
        "How did Muddy Waters demonstrate Sino-Forest's timber claims were physically impossible?",
      options: [
  ...
  },
  {
    ...
    layer0:
      "Gibney's argument was that venture capital in the 2000s had systematically shifted away from funding ambitious, physically hard technology — biotech, energy, aerospace, transportation — toward funding cheap, fast-to-build consumer internet and mobile apps, because those were lower-risk, faster to exit, and easier for VCs to evaluate. The manifesto framed this as a civilizational loss disguised as an industry success story: venture was making money, but on comparatively trivial problems, while the harder, more transformative technologies that could genuinely change material human life went underfunded.",
    layer1:
      "The mechanism is a structural incentive mismatch inside the venture capital funding model itself. Software startups require little capital, can iterate and ship in weeks, and can exit through acquisition or IPO on a timeline that matches a typical 10-year fund cycle — while 'hard tech' (aerospace, energy, biotech) requires enormous upfront capital, faces long regulatory and R&D timelines, and doesn't fit neatly into that same fund cycle. Gibney argued this incentive structure quietly steered an entire generation of venture capital toward funding whatever was fastest and cheapest to build and exit, regardless of how transformative it actually was — and Founders Fund explicitly positioned itself as a contrarian bet on reversing that pattern (an investment posture that later shaped its early bets on SpaceX and Palantir).",
    layer2:
      "Look at where venture capital or your own personal effort tends to flow by default — toward what's fast and low-risk to build and ship, or toward what's actually hardest and most consequential. What would it take to deliberately bias your own effort toward the harder, slower thing?",
    quiz: {
      question:
        "What is the famous line from this manifesto that captures its central critique of venture capital?",
      options: [
  ...
  },
  {
    ...
    layer2:
      "Pick one of ARK's flagged 'disruptive platforms' (EVs, gene editing, humanoid robots, AI) and try to find an actual cost-per-unit trend for it over the last 5-10 years. Does the data support a Wright's Law-style consistent decline, or has the trend been bumpier than a clean forecast would suggest?",
    quiz: {
      question:
        "What empirical pattern does ARK Invest's forecasting method rely on to project technology adoption?",
      options: [
  ...
  },
  {
    ...
    layer0:
      "In 1995, almost nobody on Wall Street had a systematic, data-driven way to evaluate internet companies — there was no established playbook for valuing businesses built on a technology this new. Meeker's contribution was simply doing the unglamorous work of aggregating usage data, infrastructure growth, and adoption curves into one document, giving investors an actual evidentiary basis for internet-related investment decisions instead of pure speculation or narrative.",
    layer1:
      "The mechanism was establishing a repeatable, data-first analytical format in a domain where none existed: rather than relying on qualitative pitches from founders, Meeker's report tracked hard usage metrics (users online, time spent, infrastructure buildout, later mobile and ad spend data) year over year, letting investors see actual growth trajectories rather than assumed ones. This turned an annual slide deck into a genuine reference dataset that shaped how an entire generation of technology investors thought about measuring adoption.",
    layer2:
      "Think of an emerging technology today that lacks a clear, agreed-upon set of adoption metrics the way the early internet once did. What data would you need to track consistently, year over year, to build the kind of evidentiary base Meeker built for the internet?",
    quiz: {
      question:
        "What made Mary Meeker's original Internet Trends work distinctive within Wall Street analysis at the time?",
      options: [
  ...
  },
  {
    ...
    quiz: {
      question:
        "What economic argument does Karnofsky use to support the idea that transformative AI could make this the most important century in history?",
      options: [
  ...
  },
  {
    ...
    layer2:
      "Pick one of the three trends Aschenbrenner compounds (compute scaling, algorithmic efficiency, or 'unhobbling') and try to find actual data on its pace over the last two years. Is it tracking with his projected rate, running ahead, or falling behind?",
    quiz: {
      question:
        "What follow-up assessment did the 'AI 2027' scenario-planning project make of Aschenbrenner's original timeline as of early 2026?",
      options: [
